...
function buildPrompt(parsed, steamAppId, manualSize, manualCoverUrl) {
  const urls = [];
  ...
  // 游戏大小：文本自动识别 > 用户手填 > 占位提示
  const gameSize = (parsed.size || manualSize || "").trim();
  const sizeLine = gameSize || "【从夸克分享页抓取，如 30.7G】";

  // 封面图：统一由 bl 联网搜索封面直链并下载；Steam 也可用 cloudflare CDN；非 Steam 无可靠直链时由用户手填链接兜底
  const coverLine = steamAppId
    ...
    : `封面由 bl 联网搜索官方封面直链下载；若 bl 无法获得可靠直链，可在工具「封面链接」输入框粘贴图片 URL：${manualCoverUrl || "（未提供）"}`;
