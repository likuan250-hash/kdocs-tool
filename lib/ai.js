...
function runCmd(cmd, opts = {}) {
  try { return execSync(cmd, { encoding: "utf-8", maxBuffer: 10 * 1024 * 1024, shell: true, windowsHide: true, ...opts }); }
...
function aiDescribe(gameName, rawLine, opts = {}) {
  ...
  const baiduUrl = opts.baiduUrl || "";
  try {
    const out = run(`bl text chat --message ${JSON.stringify(buildPrompt(gameName, opts))} --max-tokens 600 --output json 2>&1`, { timeout: 60000 });
    ...
    if (r1.badIntro || r1.badSize || r1.badCover) {
      ...
      try {
        const out2 = run(`bl text chat --message ${JSON.stringify(retryPrompt)} --max-tokens ${r1.badIntro ? 600 : 300} --output json 2>&1`, { timeout: 45000 });
