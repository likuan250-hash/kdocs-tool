...
async function checkBlAvailable() {
  try { await runCmd("bl --version 2>&1", { timeout: 3000 }); return true; }
  catch { return false; }
}
...
async function aiDescribe(gameName, rawLine, opts = {}) {
  ...
  const baiduUrl = opts.baiduUrl || "";
  try {
    const out = await run(`bl text chat --message ${JSON.stringify(buildPrompt(gameName, opts))} --max-tokens 600 --output json 2>&1`, { timeout: 60000 });
    ...
    if (r1.badIntro || r1.badSize || r1.badCover) {
      ...
      try {
        const out2 = await run(`bl text chat --message ${JSON.stringify(retryPrompt)} --max-tokens ${r1.badIntro ? 600 : 300} --output json 2>&1`, { timeout: 45000 });
