...
const { searchCoverByGameName } = require("./coverdb");
...
async function autoExecute(parsed, manualAppId, coverDir, opts = {}) {
  ...
  const ai = aiDescribe(parsed.gameName, parsed.raw, {
  ...
  let desc = ai.intro && !blacklist.test(ai.intro) ? ai.intro : "";
  ...
  if (!coverPath && parsed.gameName) {
    ...
    try {
      const sgdbUrl = await searchCoverByGameName(parsed.gameName);
      if (sgdbUrl) {
        coverPath = await downloadCoverFromUrl(parsed.gameName, sgdbUrl, coverDir);
        const s = fs.statSync(coverPath);
  ...
  if (!coverPath) skip({ name: "封面下载", reason: "bl 未找到封面且非 Steam 且无手动链接" });
