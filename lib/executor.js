...
async function autoExecute(parsed, manualAppId, coverDir, opts = {}) {
  ...
  if (!(await deps.checkKdocsReady())) {
    ...
    const result = { steps, recordId: null, success: false };
  ...
  try {
    await deps.callMcporter("dbsheet.list_records", { sheet_id: 1 });
    ok({ name: "查重完成" });
  ...
  const result = { steps, recordId, success, gameName: parsed.gameName };
  emit({ type: "done", result });
  return result;
}
